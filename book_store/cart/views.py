...
def add_cart(request,books_id):
    prod = books.objects.get(id=books_id)
    user = request.user
    ...
    try:
        c_item=cartitems.objects.get(books=prod,cart=ct)
        if c_item.quantity < c_item.books.stock:
            c_item.quantity+=1
            prod.stock -=1
            prod.save()
        ...
        c_items=cartitems.objects.create(books=prod,quantity=1,cart=ct)
        c_item.save()
...
def min_cart(request,books_id):
    ...
    try:
        ...
        if ct_list.exists:
            for ct in ct_list:
                prod=get_object_or_404 (books,id=books_id)
                try:
                    c_items=cartitems.objects.get(books=prod, cart=ct)
...
@login_required(login_url='register')
def cart_delete(request, books_id):
    user = request.user
    try:
        if user.is_authenticated:
            ct_list = cartlist.objects.filter(user=user)
